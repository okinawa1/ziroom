...
data class Room(
    ...
    fun isOriginalRoom(): Boolean {
        return try {
            val doc = Jsoup.connect("http:${this.detail_url}").get()!!
            val ztags = doc.getElementsByClass("Z_tags")[0].children()
            ztags.none { it.text()!! == "独立阳台" }
        } catch (e: Exception) {
            println("抓取自如 detail失败, ${e.message}}")
            true
        }
    }
}
